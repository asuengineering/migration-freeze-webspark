...
function mfw_register_settings_page() {
	add_options_page( __( 'Migration Status', 'migration-freeze-webspark' ), __( 'Migration Status', 'migration-freeze-webspark' ), 'manage_options', 'mfw-migration-status', 'mfw_render_settings_page' );
}
...
function mfw_handle_state_update() {
	...
	$new_state = isset( $_POST['mfw_site_state'] ) ? sanitize_key( wp_unslash( $_POST['mfw_site_state'] ) ) : MFW_STATE_NORMAL;
...
function mfw_render_settings_page() {
	...
	?>
	<div class="wrap"><h1><?php esc_html_e( 'Migration Status', 'migration-freeze-webspark' ); ?></h1>
